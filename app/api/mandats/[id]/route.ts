...
const ALLOWED_FIELDS = [
  'clientName', 'clientEmail', 'subject', 'billing', 'priceHT',
  'contactName', 'contactPhone', 'contactEmail', 'notes', 'active',
  'clientId', 'clientAddress', 'clientPostalCode', 'clientCity', 'clientCountry',
  'clientPhone2', 'clientSiren', 'clientPrenom', 'bic', 'iban',
  'paymentType', 'referenceMandat', 'referenceContrat', 'descriptionContrat',
]
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = await prisma.mandat.findUnique({ where: { id }, select: { createdById: true } })
  ...
  const mandat = await prisma.mandat.update({ where: { id }, data })
