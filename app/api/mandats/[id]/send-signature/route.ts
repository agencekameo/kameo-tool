...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const signingUrl = `${baseUrl}/signature/mandat/${token}`
    ...
    const html = buildContractSignatureEmailHtml({
      clientName: mandat.clientName,
      subject: mandat.subject || 'Mandat de prélèvement',
      type: 'MANDAT',
      signingUrl,
    })
    ...
    await transporter.sendMail({
      ...
      to: email,
      subject: `Mandat à signer — ${mandat.subject || mandat.clientName}`,
      html,
    })

    return NextResponse.json({ success: true })
  } catch (err) {
    console.error('[POST /api/mandats/[id]/send-signature]', err)
